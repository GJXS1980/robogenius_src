...
from auto_demo.msg import object_result_msg
from std_msgs.msg import Int32MultiArray, Float32MultiArray, Int64
...
class Findposition:
    def __init__(self):
        ...
        # rospy.Subscriber('/ultrasonic_data', Float32MultiArray, self.image_det)
    ...
    # def image_det(self):        
        ...
        i = 0
        while self.tag == 8 and self.flag == 0 and color_judge:
            time.sleep(0.5)
...
#####################      获取物体的中心坐标回调函数　　   ######################
    def image_callback(self, data):
        ...
        if len(points) == 2:
            ...
            # self.side_judge = ((100 <= self.px and self.px <= 360) and (176 <= self.st[0] and self.st[0] <= 186) and (176 <= self.st[1] and self.st[1] <= 186))
            self.side_judge = ((100 <= self.px and self.px <= 360) and (120 <= self.st[0] and self.st[0] <= 240) and (120 <= self.st[1] and self.st[1] <= 240))
