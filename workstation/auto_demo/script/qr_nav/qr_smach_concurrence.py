...
class qr_Command(State):
    ...
    def execute(self, userdata):
        ...
        rospy.Subscriber('/qr_result', String , self.Get_Command)
...
class Qr_Nav_Smach():
    def __init__(self):
        ...
        # 为语音指令子任务创建一个状态机
        sm_qr_command = StateMachine(outcomes=['livingroom_id', 'bedroom_id', 'kitchen_id', 'balcony_id', 'None_id'])
        # 然后添加子任务
        with sm_qr_command:
            StateMachine.add('RECOGNIZE_QR_COMMAND', qr_Command(self.qr_id_data, 5), transitions={'livingroom_id':'', 'bedroom_id':'', 'kitchen_id':'', 'balcony_id':'', 'None_id':''})

        # 为等待任务创建一个状态机
        sm_waite_command = StateMachine(outcomes=['waite', 'preempted'])
        # 然后添加子任务
        with sm_waite_command:
            StateMachine.add('WAITE_QR_COMMAND', None_Command('waite_qr_command', 5), transitions={'waite':'', 'preempted':''})
        ...
        with self.sm_nav:
            #   语音识别任务状态机
            StateMachine.add('QR_COMMAND', sm_qr_command, transitions={'livingroom_id':'CLEAN_LIVING_ROOM', 'bedroom_id':'CLEAN_BEDROOM', 'kitchen_id':'CLEAN_KITCHEN', 'balcony_id':'CLEAN_BALCONY', 'None_id':'WAITE_QR_COMMAND'})
            ...
            StateMachine.add('CLEAN_LIVING_ROOM_TASKS', sm_living_room, transitions={'succeeded':'QR_COMMAND','aborted':'','preempted':''})
            ...
            StateMachine.add('CLEAN_BEDROOM_TASKS', sm_bedroom, transitions={'succeeded':'QR_COMMAND','aborted':'','preempted':''})
            ...
            StateMachine.add('CLEAN_KITCHEN_TASKS', sm_kitchen, transitions={'succeeded':'QR_COMMAND','aborted':'','preempted':''})         
            ...
            StateMachine.add('CLEAN_BALCONY_TASKS', sm_balcony, transitions={'succeeded':'QR_COMMAND','aborted':'','preempted':''})         
       
            ''' Add the waite qr command subtask(s) '''
            #   当没有识别到语音指令时，进入等待
            StateMachine.add('WAITE_QR_COMMAND', sm_waite_command, transitions={'waite':'QR_COMMAND', 'preempted':''})  
